...
function calculateScore(myProfile, theirProfile, theirUser) {
  ...
  // 1. Shared interests (0-40 points)
  ...
  // 3. Age preference match (0-20 points)
  ...
  // 5. Verification bonus (0-5 points)
  ...
  try {
    ...
    const result = db.query(
      `SELECT p.*, u.last_active, u.is_verified as user_verified,
       ...
       LIMIT 50`,
      [req.user.id, maxDate.toISOString().split('T')[0], minDate.toISOString().split('T')[0], ...genderParams, req.user.id]
    );
    ...
    const scored = result.rows.map(p => {
      ...
      const { score, reasons, sharedInterests } = calculateScore(myP, p, p);
      return { ...profile, matchScore: score, matchReasons: reasons, sharedInterests };
    });
